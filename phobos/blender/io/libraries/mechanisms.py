...
def compileMechanismList():
    ...
    for i, mechanism in enumerate(defs.definitions['submechanisms'], start=1):
        size = len(defs.definitions['submechanisms'][mechanism]['joints']['spanningtree'])
        imagepath = os.path.join(imagefolderpath, f'{mechanism}.png')
        if not (os.path.exists(imagepath) and os.path.isfile(imagepath)):
            ...
            log(f"Adding mechanism preview: {imagepath}", 'DEBUG')
