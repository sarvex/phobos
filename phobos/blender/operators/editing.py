...
class SafelyRemoveObjectsFromSceneOperator(Operator):
    ...
    def execute(self, context):
        ...
        log(
            f"Removed {len(objs_to_delete)} object{'s' if len(objs_to_delete) > 1 else ''} from configuration.",
            'INFO',
        )
        log(f"    Objects: {[obj.name for obj in objs_to_delete]}", 'DEBUG')
        if objs_to_keep:
            log('Some objects were not removed as they are unique to this scene.', 'WARNING')
            log(f"    Objects: {[obj.name for obj in objs_to_keep]}", 'DEBUG')
        return {'FINISHED'}
    ...
    def execute(self, context):
        ...
        if self.truecopy:
            ...
            for obj in moveobjs:
                ...
                if f'{obj.phobostype}/name' not in obj:
                    obj[f'{obj.phobostype}/name'] = name
            ...
            for newobj in newobjs:
                newobj.name = f'{newscene.name}_{nUtils.getObjectName(newobj)}'
        ...
        if self.remove:
            ...
            for obj in roots:
                ...
                log(f"Cleared parent for new object root {obj.name}", 'INFO')
        ...
        log(
            f"Added {len(moveobjs)} object{'s' if len(moveobjs) > 1 else ''} to {'new ' if self.new else ''}{'initialized ' if self.init else ''}export configuration '{self.scenename}'.",
            'INFO',
        )
        log(f"  Objects: {[obj.name for obj in moveobjs]}", 'DEBUG')
        return {'FINISHED'}
...
class CreateInterfaceOperator(Operator):
    ...
    def execute(self, context):
        ...
        if self.all_selected:
            for link in [obj for obj in context.selected_objects if obj.phobostype == 'link']:
                ifdict['parent'] = link
                ifdict['name'] = f'{link.name}_{self.interface_name}'
    ...
    def poll(cls, context):
        ...
        """
        return context.object.mode == 'OBJECT' if context.object else True
...
class SetGeometryType(Operator):
    ...
    def execute(self, context):
        ...
        for obj in objs:
            if obj.phobostype in ['collision', 'visual']:
                obj['geometry/type'] = self.geomType
            else:
                log(f"The object '{obj.name}' is no collision or visual.", 'WARNING')

        log(
            (
                f"Changed geometry type for {len(objs)} object{'s' if len(objs) > 1 else ''}"
                + f" to {self.geomType}."
            ),
            'INFO',
        )
        log(f"    Objects: {[obj.name for obj in objs]}", 'DEBUG')
        return {'FINISHED'}
...
class EditInertialData(Operator):
    ...
    def execute(self, context):
        ...
        if self.changeMass:
            log(f"Changed mass to {str(newmass)}" + f" for {len(objs)} objects.", 'INFO')
        if self.changeInertia:
            log(
                f"Changed inertia to {str(newinertia)}"
                + f" for {len(objs)} objects.",
                'INFO',
            )
        return {'FINISHED'}
...
class GenerateInertialObjectsOperator(Operator):
    ...
    def invoke(self, context, event):
        ...
        self.derive_inertia_from_geometry = bool(geometric_objects)
    ...
    def execute(self, context):
        ...
        if self.clear:
            ...
            log(f"Removing old inertial objects: {inertial_objects}", 'DEBUG')
        ...
        for obj in objectlist:
            ...
            if newinertial := inertialib.createInertial(
                inertialdict, obj, adjust=True, logging=True
            ):
    ...
    def poll(cls, context):
        ...
        return context.selected_objects is not None and bool(
            (
                ...
                & {o.phobostype for o in context.selected_objects}
            )
        )
...
class EditYAMLDictionary(Operator):
    ...
    def execute(self, context):
        ...
        for key, value_ in tmpdict.items():
            # transform Blender id_arrays into lists
            if hasattr(value_, 'to_list'):
        ...
        contents = [
            ...
            "import json",
            "import bpy",
            f"tmpdata = json.loads({variablename})",
    ...
    def execute(self, context):
        ...
        for joint in (obj for obj in context.selected_objects if obj.phobostype == 'link'):
            ...
            if self.passive:
                ...
                log(f"Please add motor to active joint in {joint.name}", "INFO")
        return {'FINISHED'}
...
class AddMotorOperator(Operator):
    ...
    def poll(cls, context):
        ...
        return 'joint/type' in active_obj and active_obj['joint/type'] != 'fixed'
...
def addMotorFromYaml(motor_dict, annotations, selected_objs, active_obj, *args):
    ...
    for joint in joints:
        ...
        annotation_objs.extend(
            eUtils.addAnnotationObject(
                motor_obj,
                annotations[annot],
                name=f'{nUtils.getObjectName(motor_obj)}_{annot}',
                namespace=f'motor/{annot}',
            )
...
def addSensorFromYaml(sensor_dict, annotations, selected_objs, active_obj, *args):
    ...
    parent_obj = newlink if newlink else parentlink
    ...
    annotation_objs = [
        eUtils.addAnnotationObject(
            sensor_obj,
            annotations[annot],
            name=f'{nUtils.getObjectName(sensor_obj)}_{annot}',
            namespace=f'sensor/{annot}',
        )
        for annot in annotations
    ]
...
class AddSensorOperator(Operator):
    ...
    def categorylist(self, context):
        ...
        categories = list(defs.def_subcategories['sensors'])
    ...
    def execute(self, context):
        ...
        if not self.addLink and context.active_object.phobostype != 'link':
            log(
        ...
        if operatorPattern.match(opName):
            eval(f"bpy.ops.{opName}('INVOKE_DEFAULT')")
        else:
            log(
    ...
    for obj in objects:
        ...
        annotation_objs.extend(
            eUtils.addAnnotationObject(
                controller_obj,
                annotations[annot],
                name=f'{nUtils.getObjectName(controller_obj)}_{annot}',
                namespace=f'controller/{annot}',
            )
            for annot in annotations
        )
...
class AddControllerOperator(Operator):
    ...
    def categorylist(self, context):
        ...
        phobosIcon = prev_collections["phobos"]["phobosIcon"].icon_id
        categories = list(defs.def_subcategories['controllers'])

        icon = ''
        items = []
        for i, categ in enumerate(categories):
            # assign an icon to the phobos preset categories
            icon = 'AUTO' if categ == 'motor' else ''
            items.append((categ, categ, categ, icon, i))
        return items
...
class AddHeightmapOperator(Operator):
    ...
    def execute(self, context):
        ...
        plane.name = f'{self.name}_visual::heightmap'
        root.name = f"{self.name}::heightmap"
...
class AddSubmodel(Operator):
    ...
    def submodeltypes(self, context):
        ...
        return [(a, a, f'{a} submodel type') for a in submodellist]
    ...
    def poll(cls, context):
        ...
        """
        return any('submodeltype' in group for group in bpy.data.groups)
...
class AssignSubmechanism(Operator):
    ...
    def isLinearChain(self, jointlist):
        ...
        leafs = [
            ...
            if all(c not in jointlist for c in joint.children)
        ]
        if len(leafs) != 1:
            return jointlist
        count = 1
        ...
        if not self.linear_chain:
            ...
            if size == len(self.joints):
                ...
                for i in range(size):
                    ...
                    c2.prop(self, f"jointtype{str(i)}", text='')
            else:
        ...
        if self.mechanism_name:
            if self.linear_chain:
                ...
                if len(self.joints) == size:
                    jointmap = {
                        getattr(self, f'jointtype{str(i)}'): self.joints[i]
                        for i in range(len(self.joints))
                    }
                    # assign attributes
                    try:
                        for i in range(len(self.joints)):
                            self.joints[i]['submechanism/jointname'] = getattr(self, f'jointtype{str(i)}')
...
class SelectSubmechanism(Operator):
    ...
    def poll(cls, context):
        ...
        return bool(sUtils.getSubmechanismRoots())
...
class DisconnectInterfaceOperator(Operator):
    ...
    def poll(cls, context):
        ...
        if (
            ...
            or context.active_object.phobostype != 'interface'
        ):
            return False
        ...
        if interface.parent and interface.parent.phobostype == 'interface':
            return True
        elif interface.children and any(
            obj.phobostype for obj in interface.children
        ):
            return True
        return False
        ...
        if interface.parent and interface.parent.phobostype == 'interface':
            ...
            for curchild in interface.children:
                if curchild.phobostype == 'interface':
                    child = curchild
                    break
            log(f'Selected {child.name} as child.', 'DEBUG')
...
class ValidateOperator(Operator):
    ...
    def execute(self, context):
        ...
        vUtils.checkMessages = messages if list(messages.keys()) else {"NoObject": []}
        for entry, value in messages.items():
            log(f"Errors in object {entry}:", 'INFO')
            for error in value:
                log(error, 'INFO')
        return {'FINISHED'}
...
class CalculateMassOperator(Operator):
    ...
    def invoke(self, context, event):
        ...
        log(f"The calculated mass is: {str(self.mass)}", 'INFO')
    ...
    def draw(self, context):
        ...
        """
        self.layout.label(text=f"Sum of masses: {str(self.mass)}")
...
class MeasureDistanceOperator(Operator):
    ...
    def execute(self, context):
        ...
        log(f"distance: {str(self.distance)}, {str(self.distVector)}", "INFO")
        return {'FINISHED'}
